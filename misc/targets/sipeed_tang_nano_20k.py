#!/usr/bin/env python3
"""
Tang Nano 20K board support for LiteX.
Contains CRG, BaseSoC and build functionality for the Sipeed Tang Nano 20K FPGA board.
"""
...
from migen import Signal, ClockSignal, ClockDomain, Instance, If
...
from litedram.modules import M12L64322A  # TODO: use the real model number
from litedram.phy import GENSDRPHY

try:
    from litespi.modules import W25Q64 as SpiFlashModule
    from litespi.opcodes import SpiNorFlashOpCodes as SpiNorCodes
    LITESPI_AVAILABLE = True
except ImportError:
    LITESPI_AVAILABLE = False
...
class _CRG(LiteXModule):
    """Clock and Reset Generator for Tang Nano 20K."""

    def __init__(self, platform, sys_clk_freq, with_hdmi=False):
        """Initialize CRG with system clock and optional HDMI clocks."""
        self.rst = Signal()
...
class BaseSoC(SoCCore):
    """Base SoC for Tang Nano 20K with configurable peripherals."""

    def __init__(self, toolchain="gowin", sys_clk_freq=48e6,
                 with_led_chaser=True, with_rgb_led=False, with_buttons=True,
                 with_spi_flash=False, with_video_terminal=False,
                 with_video_colorbars=False, **kwargs):
        """Initialize BaseSoC with specified configuration."""
        ...
        SoCCore.__init__(
            ...
            ident="Zephyr SoC on Tang Nano 20K by SantaCRC (Fabian Alvarez)",
            **kwargs
        )

        # SPI Flash: XT25F64B ----------------------------------------------------------------------
        if with_spi_flash:
            if LITESPI_AVAILABLE:
                self.add_spi_flash(
                    mode="1x", module=SpiFlashModule(SpiNorCodes.READ_1_1_1)
                )
            else:
                print("Warning: litespi not available, SPI flash disabled")

        # SDR SDRAM --------------------------------------------------------------------------------
        if not self.integrated_main_ram_size:
            class SDRAMPads:
                """SDRAM pad definitions for Tang Nano 20K."""

                def __init__(self):
                    """Initialize SDRAM pads."""
                    ...
                    self.a = platform.request("O_sdram_addr")  # pylint: disable=invalid-name
                    self.ba = platform.request("O_sdram_ba")  # pylint: disable=invalid-name
                    self.dq = platform.request("IO_sdram_dq")  # pylint: disable=invalid-name
            ...
            self.add_sdram("sdram",
                phy=self.sdrphy,
                module=M12L64322A(sys_clk_freq, "1:1"),  # TODO: verify module
                l2_cache_size=128,
            )

        # Video ------------------------------------------------------------------------------------
        if with_hdmi:
            self.videophy = VideoGowinHDMIPHY(
                platform.request("hdmi"), clock_domain="hdmi"
            )
            if with_video_terminal:
                self.add_video_terminal(
                    phy=self.videophy, timings="640x480@60Hz",
                    clock_domain="hdmi"
                )
            if with_video_colorbars:
                self.add_video_colorbars(
                    phy=self.videophy, timings="800x600@60Hz",
                    clock_domain="hdmi"
                )
        ...
        if with_rgb_led:
            ...
            self.bus.add_slave(
                name="rgb_led", slave=self.rgb_led.bus,
                region=SoCRegion(origin=0x2000_0000, size=4)
            )
...
def main():
    """Main function for building and programming the Tang Nano 20K."""
    parser = LiteXArgumentParser(
        platform=sipeed_tang_nano_20k.Platform,
        description="LiteX SoC on Tang Nano 20K."
    )
    ...
    parser.add_target_argument("--with-rbg-led", action="store_true",
                             help="Enable WS2812 RGB Led.")
